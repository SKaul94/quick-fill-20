...
export const config = {
  version: '2.0',
  debug: false, // reduces performance
  log: true, // log to console
  prefill: true,  // prefill case editor values with default data for testing
  catalogue_all_fields: true, // show collection of all fields of all PDFs together
  profileIdentifier: 'all_rules',  // key in localStorage, update for every new version with rule changes
  rulesShouldBeDeletable: false,
  ...
  yesValues: [ 'Yes', 'Ja', '1' ],
  // OPEN_DELAY: 200, // Timer in ms to wait after opening a PDF, see index.js
  ...
  initialRulesList: {
    ...
    Allgemein: [
      ...
      {rule_type: "equal", rule: "f.ort.1", value: "${Ort}$"},
      ...
      {rule_type: "equal", rule: "f.datum.1, f.dat.1, f.datum_datum", value: "${heute}$"},
      ...
      {rule_type: "equal", rule: "f.bearbeiter.1, f.sachbearbeiter, f.sachbearbeiter.1, f.sachbe.1, f.ersteller.1", value: "${Dienstrang}$ ${BPOL-Diensthabender}$"},
      ...
      {rule_type: "equal", rule: "f.vorgangsnr.1, f.vorgangsnummer.1, f.vgnummer.1", value: "${Vorgangsnummer}$"},
      ...
      {rule_type: "date", rule: 'Geburtsdatum', value: '${personalie.geburtsdatum}$' },  // from XML
      {rule_type: "equal", pdf: '1_00_056', rule: 'f.ort.2', value: '${Geburtsdatum}$' },
      ...
      {rule_type: "equal", rule: 'f.gebdat.1, f.gebdatum.1, f.geburtsdatum.1', value: '${Geburtsdatum}$' },
      ...
      {rule_type: "equal", rule: "f.vor.1, f.vorn.1, f.vorname.1", value: "${Vorname}$"},
      // {rule_type: "substring", rule: "vorn", value: "${Vorname}$"},

      {rule_type: "equal", rule: "Familienname", value: "${familienname.bezeichnung.2}$"},
      {rule_type: "equal", rule: "f.nach.1, f.nachn.1, f.nachname.1", value: "${Familienname}$"},
      ...
      {rule_type: "substring", rule: "staat", value: "${Staatsangehörigkeit}$"},
      ...
      {rule_type: "equal", rule: "f.unter.1, f.unterschrift.1", value: '${BPOL-Diensthabender}$, ${Dienstrang}$' },
      ...
      // ToDo Checkboxes:
      // {rule_type: "equal", rule: "f.kk.2", value: "${Wohnanschrift}$"},
      // {rule_type: "equal", rule: "f.kk.3", value: "${ledig}$"},
    ]
  },
  profileEditors: [
  ...
  ignoreFields: ['h.hinweis.1', 's.hinweis.1', 's.merkblatt'],  // List of fields in the PDF documents to be ignored
  ...
  caseRules: [ 'Tagebuchnummer','Vorgangsnummer','Sammelvorgangsnummer','Familienname','Vorname','Geburtsname','Geburtsdatum','Geburtsort','Geburtsland','Familienstand','Staatsangehörigkeit', 'Beruf', 'PGÜ-Telefon', 'Straße', 'Hausnummer', 'Postleitzahl', 'Wohnort', 'Staat-des-Wohnorts', 'Tatort', 'Straftat-Beginn', 'Kurzbeschreibung' ],
  ...
  ID: key => key,  // field name
  heute: _=> (new Date).toLocaleString('de').split(',')[0].split('.').map(x => parseInt(x)>99?x:('0'+x).slice(-2)).join('.'), // "19.07.2024"
  jetzt: _=> (new Date).toLocaleString('de'), // "19.7.2024, 14:48:56"
  uhrzeit: _=> (new Date).toLocaleString('de').split(',')[1].trim(), // "14:48:56"

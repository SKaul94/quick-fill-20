...
export const config = quickFillConfig ? quickFillConfig : {
  ...
  initialRulesList: {
    PVB: [
      ...
      {"rule_type":"equal","rule":"f.amtsbez.1","value":"${BPOL-Diensthabender}$, ${Dienstrang}$","owner":"PVB"},
    ],
    DGL: [
    ...
    Allgemein: [
      ...
      {rule_type: "equal", rule: "f.tgbnr.1, f.vorgangsnr.1, f.vorgangsnummer.1, f.vgnummer.1, f.checkliste_vorgangsnummer, Vorgangsnummer", value: "${Vorgangsnummer}$"},
  ...
  pdfPerPerson: {  // Syntax: (menu button label): (file name prefix without ".pdf")
    ...
    refusal: { // === PDFs for refusal of entry ===
      "Anhörung zur Einreiseverweigerung":"BRAS_120_vs_nfd QF",
      "Einreiseverweigerung": "1_10_154a QF",
      "Benennung eines Empfangsbevollmächtigten": "1_90_119a QF",
    },
    identity: { // === PDFs for identity verification ===
      "Erklärung": "1_00_152a QF",
      "Belehrung": "1_00_171a QF",
      "Benennung": "1_00_134a QF",
    }
  },
  ...
  pdfInitialLoading: [ // Syntax: filename or https-URL
    'identity.zip' // identity.zip is loaded from origin on start 
  ],

  ...
  inferRules(){
    ...
    for ( const value of generalRuleValues ){
      ...
      for ( const triple of specialTriples ){
        Rule.createNew({rule_type: 'equal', rule: triple.field, value: triple.value, owner: this.owner, pdf: triple.pdf.split('.')[0]});
      }
      Rule.createNew({rule_type: 'equal', 
...
export class ProfileEditor  {
  ...
  get rules_area(){
    if ( ! this._rules_area ){
      this.root.innerHTML = document.getElementById( this.templateId ).innerHTML;
      this._rules_area = this.root.querySelector('.rules_area');
    }
    return this._rules_area;
  }

  set rules_area( area ){
    this._rules_area = area;
  }
  ...
  async plusListener( event ) {
    const newRule = Rule.createNew({ rule_type: 'equal', rule: 'neuer Wert', owner: this.owner });
    const newRow = newRule.makeRow();
    this.root.querySelector('tbody').insertBefore(newRow, this.plusButtonRow() );
    setSelection(newRow);
    newRule.installListeners();
    // do not save case or personal infos, might be sensitive
    if ( ! newRule.isCaseRule() && ! newRule.person ) saveInitialLists({silent: true});
  }
  ...
  render(){
    ...
    if ( this.person ) this.root.querySelector('.person_name').innerText = this.person.name;

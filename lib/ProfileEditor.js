...
import { addSinglePDF } from "../index.js";
  ...
  learnListener() {
    ...
    class SetOfTriples extends Set {
      constructor(){
        ...
        this.mapFieldNameToValue = {}; // ToDo remove
        this.fieldNamesWithUniqueValue = new Set(); // ToDo remove
        this.allPdfs = new Set(); // ToDo remove
      ...
      add(triple) {
        ...
        if (!this.mapFieldNameToValue[field]) this.mapFieldNameToValue[field] = new Set();
        this.mapFieldNameToValue[field].add(triple);
        this.allPdfs.add(pdf);
        this.allValues.add(value);
      }
      inferRules(){
        // First rule in the list of rules wins. Order is priority. 
        for (const value of this.allValues) {
          // first rules for special cases treated with higher priority
          for (const triple of this){
            // skip triples with same value that are handled by a more general rule, see below
            if (value === triple.value && 
              Array.from(this.mapValueToSetOfFieldNames[value]).map(t => t.field).includes(triple.field)) continue; // already handled
            // exceptions of the more general rules are
            //    fields with different value that are not handled by a more general rule
            if (triple.value !== value){
              new Rule({rule_type: 'equal', rule: triple.field, value: triple.value, owner: self.name, pdf: triple.pdf.split('.')[0]});
            }
          }
          // second rule for general case with lower priority
          new Rule({rule_type: 'equal', 
            rule: Array.from(this.mapValueToSetOfFieldNames[value]).map(triple => triple.field).join(', '), 
            value, 
            owner: self.name, 
            pdf: Array.from(this.mapValueToSetOfFieldNames[value]).map(triple => triple.pdf.split('.')[0]).join(', ')});
        }
      }
    }
    ...
    return async (event) => {
      ...
      await learnRulesFromMultiplePDFs(dirHandle.values());

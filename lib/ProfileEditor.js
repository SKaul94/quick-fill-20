...
export class ProfileEditor  {
  ...
  save(){  // ToDo identical to Rule.DB.store(). Duplicate method?
    localStorage.setItem( config.profileIdentifier, JSON.stringify( Rule.DB.toJSON() ) );
    config[ config.profileIdentifier ] = Rule.DB.toJSON();
  }
  ...
  displayOwnRules(){
    ...
    if ( this.owner === 'template' ){
      for ( const template of config.initialRulesList.template ){
        const templateRule = new Rule( template );
        templateRule.owner = 'template';
        templateRule.active = false; // templates should never get active
        templateRule.makeRow();
        tbody.insertBefore( templateRule.row, this.plusButtonRow() );
        templateRule.installListeners();
      }
    }
    for ( const rule of Rule.DB.sortedRules() ) {
      if ( rule.owner !== this.owner ) continue; // separate ProfileEditor for every owner
  ...
  update(){
    // insert new values
    for ( const rule of Rule.DB.sortedRules().filter( rule => rule.owner === this.owner ) ) {

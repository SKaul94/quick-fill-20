...
export class Field  {
  ...
  toTableRow() {
    ...
    switch( this.typ ){
      case 'text':
        ...
        break;
      case 'checkbox': case "radiobutton": 
        const checkboxes = this.exportValues.map( (exportValue, i) => `<label>${exportValue}<input type="${( this.exportValues.length === 1 )?'checkbox':'radio'}" name="${makeClassName(this.name)}" ${exportValue === this.value?" checked":""}></label><br>` ).join('');
  ...
  whileEnabledDo( action ){
    const disableStatusBefore = [];
    const hiddenStatusBefore = [];
    for ( const elem of this.htmlElements ){
      disableStatusBefore.push( elem.hasAttribute( 'disabled' ) );
      hiddenStatusBefore.push( elem.hasAttribute( 'hidden' ) );
      elem.disabled = false;
      elem.setAttribute('hidden', false);
      action();
    }
    this.pdfDoc.applyTableToPDF();
    this.htmlElements.forEach( (elem,i) => {
      elem.disabled = disableStatusBefore[i];
      elem.setAttribute('hidden', hiddenStatusBefore[i]);
    });    
  }
    ...
    switch ( this.typ ){
      case 'text':
        inputField.addEventListener('blur', async event => {
          this.whileEnabledDo( _=> this.setValueInPdf( inputField.textContent ) );
        });
        break;
      case 'checkbox': case "radiobutton":
        inputField.addEventListener('change', async event => {
          const labelOfTarget = event.target.parentElement.textContent;
          this.whileEnabledDo( _=> this.setValueInPdf( labelOfTarget ) );
          // this.whileEnabledDo( _=> this.tableCheckboxClick( labelOfTarget ) );
        });
        break;
      case 'combobox': case "listbox": // ToDo same as dropdown ?
        inputField.addEventListener('change', async event => {
          ...
          this.whileEnabledDo( _=> this.setValueInPdf( labelOfTarget ) );
          // this.whileEnabledDo( _=> this.select( labelOfTarget ) );
        });
        break;
      case 'dropdown': // ToDo same as combobox ?
        inputField.addEventListener('change', async event => {
          this.whileEnabledDo( _=> {
            // this.pdfField.select( inputField.value );
            this.setValueInPdf( inputField.value );
          } );
        });
        break;
      case 'button':
  ...
  select( labelOfTarget ){
    ...
    this.pdfDoc.pdfDocumentInstance._annotationStorageModified = true;
  }

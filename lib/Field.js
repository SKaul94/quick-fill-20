...
export class Field  {
  ...
  setValueInPdf( value ){  // allow to set value to empty string
            ...
            if ( pdfObject.type === this.typ ){
              ...
              if ( htmlElement ){
                htmlElement.addEventListener('focus', event => {
                  // PDF internal script stores old content and overwrites QuickFill data.
                  // Therefore restore QuickFill data in PDF.
  ...
  toHTMLInputField(){
    ...
    switch ( this.typ ){
      ...
      case 'checkbox': case 'combobox': return this.exportValues.map( exportValue => `<span>${exportValue}<input type="checkbox" id="${makeClassName(this.name+exportValue)}"></span>` ).join(''); 

...
export class Field  {
  ...
  get value(){  // Rules with highest prority, then table, then PDF
    if ( this._value == null  ){ // undefined or null
      if ( this._dirty ){  // do not use rules if dirty
        this._value = this.valueFromTable() || this.valueFromPDF() || '';
      } else {
        this._value = this.valueFromRules() || this.valueFromTable() || this.valueFromPDF() || '';
      }
    }
    return this._value;
  }
  ...
  // transfer value to PDF, by rule or manually 
  setValueInPdf( value, tableElement ){  // allow to set value to empty string
    ...
    switch ( this.typ ){
      ...
      //   for ( const pdfObject of this.pdfObjects ){
      //     if ( pdfObject.type === this.typ ){
      //       const checkedElement = tableElement.querySelector('input:not(.dirty):checked');
      ...
      //   }
      //   break;
      case 'checkbox': case 'radiobutton': // like checkbox, but uncheck all other radio buttons
        for ( const pdfObject of this.pdfObjects ){
          if ( pdfObject.type === this.typ ){
            const checkedElement = tableElement.querySelector('input:not(.dirty):checked');
            let result = checkedElement ? true : false;
            if ( checkedElement ) value = checkedElement.parentElement.textContent;
            if ( pdfObject.exportValues === value ){
              pdfObject.value = checkedElement ? 'On' : 'Off';
            } else {
  ...
  valueFromTable(){
    ...
    switch( this.typ ){
      ...
      case 'combobox': case 'listbox': return this.htmlRow.querySelector('.value select').value;

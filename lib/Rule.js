...
export class Rule  {
  constructor({rule_type, rule, value, threshold, caseRule, pdf, owner, person}) {
    ...
    this._rule = typeof rule === 'object' ? rule.name : rule;
    this._value = value;
    ...
    if ( ! Rule.DB.contains(this)) Rule.DB.addRule(this);  // every Rule is in the Rule Database automatically
    this.check();
  }
  ...
  toString(){
    ...
    return ` Rule${this.nr}(${this.rule_type} ${this.rule_type === 'formula' ? this.rule : '"'+this.rule+'"'} "${this.value}"${pdf}) `;
  }

  prettyPrint(){
    const pdf = this.pdf ? ` (PDF ${this.pdf})` : '';
    const value = this.value ? `"${this.value}"` : '';
    return `(${this.rule_type} ${this.rule} ${value}${pdf})`;
  }
  ...
  static caseOptions(){ // options are specified by HTML declarations
    const caseDiv = document.querySelector('#case_template').content.cloneNode(true);
    const caseOptions = {
      nr:  !! caseDiv.querySelector('th.nr'),
      typ: !! caseDiv.querySelector('th.typ'),
      pdf: !! caseDiv.querySelector('th.pdf'),
      del: !! caseDiv.querySelector('th.del')
    };
    return caseOptions;
  }
  ...
  makeRow(){
    ...
    if ( this.owner === 'case' ){
      this._row = this.toCaseRow( false, this.rule, Rule.caseOptions() );
    } else {
      this._row = this.toRow();
    }
    return this._row;
  }
  ...
  toRow() {  // convert to HTML
    if ( this.isCaseRule() ) return this.toCaseRow( false, this.rule, this.caseOptions() );
  ...
  toCaseRow( withRuleEditing, caseRule, options ) {  // convert to 2-column HTML row with equal rule
    ...
    const nr_td  = options.nr  ? `<td class="nr">${this.nr}</td>`  : ''; // ToDo other columns
    const typ_td = options.typ ? `<td class="typ">${this.typ}</td>` : '';
  ...
  drop(){  // Drop Target Zone Handler
    return event => {
      ...
      if ( Rule.dragSourceNr !== this.nr ){
        const sourceRow = Rule.dragSource.getRow();
        const targetRow = this.row;
        const table = targetRow.parentElement;
        if ( Rule.dragSourceNr > this.nr ){
          table.insertBefore(sourceRow,targetRow);
          for ( const rule of Rule.DB.ruleRange( { lowerBound: this.nr } ) ){
            if ( rule.nr > 1000 ) debugger;
            if ( rule !== Rule.dragSource ) rule.nr += 1;
          }
        } else {
          // insert after
          table.insertBefore(sourceRow,targetRow.nextElementSibling);
          for ( const rule of Rule.DB.ruleRange( { upperBound: this.nr } ) ){
            if ( rule.nr < 0 ) debugger;
            if ( rule !== Rule.dragSource ) rule.nr -= 1;
          }
        }
        Rule.DB.store();
  ...
  installListeners(){
    ...
    if ( delElement ) delElement.addEventListener( 'click',  event => {
      this.removeRow();
    });
    ...
    ruleElement.addEventListener('blur', this.blurListener( 'rule' ) );
    ruleElement.addEventListener('keydown', this.keyListener( 'rule' ) );
    ruleElement.addEventListener('focus', this.focusListener( 'rule' ) );

    valueElement.addEventListener('blur', this.blurListener( 'value' ) );
    valueElement.addEventListener('keydown', this.keyListener( 'value' ) );
    valueElement.addEventListener('focus', this.focusListener( 'value' ) );

    if ( pdfElement ){
      pdfElement.addEventListener('blur', this.blurListener( 'pdf' ) );
      pdfElement.addEventListener('keydown', this.keyListener( 'pdf' ) );
      pdfElement.addEventListener('focus', this.focusListener( 'pdf' ) );
    }
  }
  ...
  blurListener( category ){
    return event => {
      ...
      if ( this[ category ] !== keyElement.innerText ){
        ...
        if ( ! keyElement.innerText ){  // empty input => delete
          if ( ! this._markAsDeleted && config.rulesShouldBeDeletable && confirm('Ganze Zeile löschen?') ) {
   ...
   */
  removeListener(){  // ToDo unify with blurListener
    return event => {
      event.stopImmediatePropagation();
      const row = this.row;
      Rule.lastActiveRow = row;

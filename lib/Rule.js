...
export class Rule  {
  constructor({rule_type, rule, value, threshold, caseRule, pdf, owner}) {
  ...
  toCaseRow( withRuleEditing, caseRule, options ) {  // convert to 2-column HTML row with equal rule
    ...
    const typ_td = options.typ ? `<td class="nr">${this.typ}</td>` : '';
    const pdf_td = options.pdf ? `<td class="nr">${this.pdf}</td>` : '';
    const del_td = options.del ? `<td class="nr">${this.del}</td>` : '';
    row.innerHTML = `${nr_td}<td class="rule" ${withRuleEditing ? 'contenteditable' : ''}>${this.rule || 'neue Regel'}</td>`;

...
export class Rule  {
  constructor({rule_type, rule, value, threshold, caseRule, pdf, owner, person, header}) {
    ...
    if ( rule ) console.assert( typeof rule === 'string' );
    this._rule = rule;
    this._value = value;
    ...
    if ( this.owner === 'template' ) this.nr = Rule.DB._templateCounter++;
    console.assert( this.rule );
    console.assert( this.check() );
  }

  static createNew({rule_type, rule, value, threshold, caseRule, pdf, owner, person, header}){
    const newRule = new Rule({rule_type, rule, value, threshold, caseRule, pdf, owner, person, header});
    const oldRule = Rule.DB.contains( newRule );
    if ( oldRule ){
      return oldRule;
    } else {
      Rule.DB.addRule( newRule );
      console.assert( newRule.nr );
      return newRule;
    }
  }
  ...
  isPersonRule(){
    return this.person || this.owner === 'person';
  }
  ...
  isGeneralRule(){
    return ! this.person && this.owner !== 'case' && this.owner !== 'person' && this.owner !== 'template';
  }
  ...
  static refocus(){
    if ( Rule.lastActiveRow ){
      console.assert( Rule.lastActiveRow.tagName === 'TR' );
  ...
  toString(){
    ...
    return ` ${kindOf}${this.nr}(${this.rule_type} ${this.rule_type === 'formula' ? this.rule : '"'+this.rule+'"'} "${this.value}"${pdf}${header}) `;
  }
  ...
  prettyPrint(){
    ...
    return `(${[this.id(), this.rule_type, this.rule, value, pdf, person, header].filter(Boolean).join(' ')})`;
  }
  ...
  toJSON() {
    ...
    if ( this.header ) result.header = this.header;
    return result;
  }
  ...
  installListeners(){
    if ( ! this.isGeneralRule() ) return this.installCaseListeners();
    const row = this.row;
  ...
  blurListener( category ){
    return (async function ( event ) {
      ...
      if ( this[ category ] !== keyElement.innerText ){
        ...
        if ( this.isCaseRule() && category === 'value' ){
          if ( this.person ) await this.person.activate();
          await PdfDoc.updateAll();
          return; 
        } 
        // do not store empty rules
        if ( this.rule && this.rule !== 'neuer Wert' && this.owner !== 'case' && this.owner !== 'person' && ! this.person ){
  ...
  installCaseListeners( withRuleEditing ){
    console.assert( ! this.isGeneralRule() );
    const row = this.row;
    const ruleElement = row.querySelector('.rule');
    const valueElement = row.querySelector('.value');
    ...
    if ( ! config.ignoreCheckboxes ) valueElement.addEventListener('click', event => {
      ...
      if ( event.target.constructor.name === 'HTMLInputElement' && event.target.type === 'radio'
        ...
        if ( this.value === event.target.value ){
          ...
          Rule.createNew({rule_type:'equal', rule: event.target.value, value: 'checked', owner: 'case' });
        }
        PdfDoc.updateAll();
       }
    });
...
if ( globalThis ){ // see https://developer.mozilla.org/de/docs/Web/JavaScript/Reference/Global_Objects/globalThis
  globalThis.top.DB = Rule.DB;  // for debugging purposes: access Rule Database in DevTools console via "DB"
}
// assuring top level window, not some nested iframe

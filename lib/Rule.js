...
export class Rule  {
  constructor({rule_type, rule, value, threshold, caseRule, pdf, owner, person}) {
    ...
    if ( ! Rule.DB.contains(this) && this.owner !== 'template' ) Rule.DB.addRule(this);  // every Rule is in the Rule Database automatically
    this.check();
  }
  ...
  makeRow(){
    ...
    this._row = this.isCaseRule() ? this.toCaseRow( false, this.rule, this.constructor.headerOptions( 'case' ) ) : this.toRow();
    return this._row;
  }

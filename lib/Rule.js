...
export class Rule  {
  ...
  isCaseRule(){
    return this.owner === 'case' || this.owner === 'person';
  }
  ...
  static headerOptions( owner ){ // options are specified by HTML declarations
    if ( ! owner ) return {};
    const template = document.querySelector('#' + owner + '_template');
    const options = {
      nr:  !! template.querySelector('th.nr'),
      typ: !! template.querySelector('th.typ'),
      pdf: !! template.querySelector('th.pdf'),
      del: !! template.querySelector('th.del')
    };
    return options;
  }
  ...
  makeRow(){
    console.assert( this.owner, `owner="${this.owner}"`);

...
export class RuleDataBase  {
  ...
  getRuleByNr( nr ){
    return this.idIndex[  'R' + parseInt( nr )  ];
  }

  getTemplateByNr( nr ){
    return this.idIndex[  'T' + parseInt( nr )  ];
  }
  ...
  addRule( newRule ){
    this._ruleCounter += 1;
    newRule.nr = this._ruleCounter;
    this.idIndex[ newRule.id() ] = newRule;
    if ( newRule.owner !== 'template' ){
  ...
  remove( oldRule ){
    this._ruleCounter -= 1;
    if ( oldRule.rule && oldRule.nr ){
      delete this._ruleIndex[ oldRule.id() ];

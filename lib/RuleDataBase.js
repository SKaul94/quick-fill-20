...
export class RuleDataBase  {
  ...
  allReplacements( field, pdfName ){
    const engine = new RuleEngine( this, field, pdfName );
    return Array.from( engine.allGen( field, pdfName ) );
  }
   ...
   * @returns {String} value - after replacements
   */
  valueFromAllRulesOf( field, pdfName ) {
    let result = RuleEngine.firstFinal( this, field, pdfName );
   ...
   * @returns {String} value - after replacements
   */
  valueFrom( from, pdfName ){
    return this.valueFromAllRulesOf( '${' + from + '}$', pdfName );
  }
   ...
   */
  maxReplaced( field, pdfName ){
    const engine = new RuleEngine( this, field, pdfName );
    return engine.maxReplaced( field, pdfName );
  }

...
export class TextBlockEditor extends ProfileEditor {
  constructor( {root, title, plus_row} ) {
    super( {owner: 'textblock', root, title, plus_row} );
    this.render();
  }
  ...
  allRuleSpecs(){
    ...
    for ( const row of this.rules_area.querySelectorAll('tbody tr.row') ) {
      const shortcut = row.querySelector('.shortcut').innerText;
      const textblock = row.querySelector('.textblock').innerText;
      ruleSpecs[ shortcut ] = textblock;
    }
    return ruleSpecs;
  }

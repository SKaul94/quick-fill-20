...
export class PersonEditor extends ProfileEditor {
  ...
  displayOwnRules(){
    ...
    for ( const ruleTemplate of TemplateEditor.instance?.allRuleSpecs() || Rule.DB.filter( r => r.person === this.person ) ){
      let rule;
      if ( TemplateEditor.instance ){
        rule = Rule.DB.find( 
        ...
        if ( ! rule ) rule = Rule.createNew({
          ...
          value: ruleTemplate.value || Rule.DB.valueFrom( ruleTemplate.rule ),

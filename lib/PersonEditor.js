...
export class PersonEditor extends ProfileEditor {
  ...
  displayOwnRules(){
    ...
    for ( const ruleTemplate of Rule.DB.filter( r => r.owner === 'template' ) ){
      ...
      valueField.addEventListener('blur', event => {
        ...
        if ( label ) label.nextElementSibling.innerHTML = event.target.textContent;

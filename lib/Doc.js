...
export class PdfDoc  {
  ...
  /**
   * @summary walk entire iframe document, even disabled nodes. querySelector does not deliver disabled nodes.
   */
  makeNewTreeWalker(){
    this.treeWalker = this.iFrame.contentDocument.createTreeWalker(this.iFrame.contentDocument.getElementById('viewer'), NodeFilter.SHOW_ALL);
  }

  /**
   * @summary generator function to deliver next node of tree walker, even disabled nodes. 
   */
  *nextNode(){
    let node = this.treeWalker.nextNode();
    while ( node ){
      yield node;
  ...
  /**
   * @summary dictionary mapping element names to elements 
   */
  get disabledFieldsDict(){
    if ( ! this._disabledFields ){
  ...
  /**
   * @summary map names of disabled elements to their elements and enable element 
   */
  enableDisabledFields(){
    this.makeNewTreeWalker();
    this._disabledFields = {};
    for ( const element of this.nextNode() ){
      if ( ['INPUT','TEXTAREA'].includes( element.tagName ) && element.hasAttribute('disabled') ){
        element.classList.add('enabled_disabled');
        element.removeAttribute("disabled");
        element.removeAttribute("hidden");
        console.log( element.name );
        this._disabledFields[ element.name ] = new Field( this, element.name, [this._pdfObjectsDict[element.dataset.elementId]], element );
      }
    }
    for ( const element of this.iFrame.contentDocument.body.querySelectorAll('.enabled_disabled') ){
      element.removeAttribute("disabled");
      element.removeAttribute("hidden");
      this._disabledFields[ element.name ] = new Field( this, element.name, [this._pdfObjectsDict[element.dataset.elementId]], element );
    }
  ...
  async renderArrayBuffer(){
    ...
    await new Promise(resolve => setTimeout(resolve, 150)); // ToDo find later event than webviewerloaded
  ...
  static invalidateAllRuleApplications(){
    console.log('invalidateAllRuleApplications');
  ...
  applyTableToPDF(){
    ...
    for ( const label of this.pdfTable.querySelectorAll('.label') ){
      ...
      if ( [ 'checkbox', 'combobox' ].includes( fieldObject.typ ) ){
        ...
        // get the label of the checked input element
        fieldObject.setValueInPdf( inputElement.parentElement.innerText );
      } else {
    ...
    // ToDo: enable disabled fields and fill them with values
    // this.makeNewTreeWalker();
    // for ( const element of this.nextNode() ){
    //   if ( ['INPUT','TEXTAREA'].includes( element.tagName ) && element.hasAttribute('disabled') ){
    //     element.classList.add('enabled_disabled');
    //     element.removeAttribute("disabled");
    //     element.removeAttribute("hidden");
    //     this.disabledFieldsDict[element.name].setValueInPdf(this.pdfTableMap.get(element.name));
    //   }
    // }
    // for ( const element of this.iFrame.contentDocument.body.querySelectorAll('.enabled_disabled') ){
    //   this.disabledFieldsDict[element.name].setValueInPdf(this.pdfTableMap.get(element.name));
   ...
   */
  static createHTMLSpace( mode = 'app'){
    const template = document.getElementById('single_result');
    const htmlSpace = document.createElement('div');
    htmlSpace.classList.add(mode); // turn visibility on in app view (or other view modes)

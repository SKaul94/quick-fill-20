...
export class PdfDoc extends EventTarget  {
  ...
  constructor( file, htmlSpace, person ) {
    ...
    this._docStates = [];
    this._dirtyMap = {}; // field -> manual value. User has overwritten field value manually.
  ...
  async installListeners(){
    ...
    this.iFrame.contentWindow.addEventListener('change',  event => { 
      ...
      if ( ! ['scaleSelect','pageNumber'].includes( event.target.id ) ) {
        ...
        if ( ['checkbox','radio','number'].includes( event.target.type ) || event.target.value ){
          this.markAsDirty( field.name, event.target.value );
        } else {
          this.unmarkAsDirty( field.name );  // empty string is falsy. Use value from rules again.
        }
  ...
  /**
   * Has user entered value into Pdf manually?
   * @param {String} fieldName (optional) - name of field into which user has entered value
   * @returns {Boolean} 
   */
  isDirty( fieldName ){
    return this._dirtyMap[ fieldName ] || Object.keys(this._dirtyMap).length > 0;
  }

  markAsDirty( fieldName, manualValue ){
    this._dirtyMap[ fieldName ] = manualValue; // user has entered value manually
    const field = this.getFieldByName( fieldName );
    field.htmlRow.classList.add('dirty');
  }

  unmarkAsDirty( fieldName ){
    delete this._dirtyMap[ fieldName ]; // user has deleted value manually
    const field = this.getFieldByName( fieldName );
    field.htmlRow.classList.remove('dirty');
  }

  getDirtyValue( fieldName ){
    return this._dirtyMap[ fieldName ];
  }
  ...
  applyRulesToTable(){  // fill table with values calculated by rules
    ...
    for ( const label of this.pdfTable.querySelectorAll('.label') ){
      ...
      if ( ! fieldObject ) continue;
      if ( this.person ) this.person.activate();
      const dirtyValue = this.dirtyValue( fieldObject.name );
      const value = config.dirtyStrategy && dirtyValue ? dirtyValue : fieldObject.valueFromRules();
      if ( ! value ) continue;
      fieldObject.value = value;
    }

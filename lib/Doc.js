...
export class PdfDoc extends EventTarget  {
  ...
  installListeners(){
    ...
    this.iFrame.contentWindow.PDFViewerApplication.eventBus.on( "updatefromsandbox", event => {
      console.log(`updatefromsandbox ${event.details} from ${event.source}`);
      // debugger;
    });

    this.iFrame.contentWindow.PDFViewerApplication.eventBus.on( "switchannotationeditorparams", event => {
      console.log(`switchannotationeditorparams ${event.details} from ${event.source}`);
      // debugger;
    });

    this.iFrame.contentWindow.PDFViewerApplication.eventBus.on( "annotationeditorstateschanged", event => {
      console.log(`annotationeditorstateschanged ${event.details} from ${event.source}`);
      this.annotationEditorUIManager = event.source;
      // debugger;
    });

...
export class PdfDoc  {
  ...
  async renderArrayBuffer( binaryData ){
    ...
    // wait for rendering
    await new Promise(resolve => 
      this.iFrame.contentWindow.PDFViewerApplication.eventBus.on( "pagerender", resolve ));
  ...
  async ready( pageNumber = this.pagesCount() ){
    return this.pageReady( pageNumber );
  }

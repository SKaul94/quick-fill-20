...
export class PdfDoc extends EventTarget  {
  ...
  static async printAllListener( event ){
    ...
    for ( const pdfDoc of PdfDoc.all ){
      if ( cancelled ) break;
      // first view, then print
      await pdfDoc.printListener( event );
    }
  ...
  async printListener( event ){
    ...
    const afterPrintListener = resolve => ev => {
      console.log('iFrame.contentWindow after print');
      // this.pdfViewerApplication.cleanup();
      this.hideAllParents( hiddenParents );
      resolve();
    };
    const afterPrintPromise = new Promise( resolve => {
      this.iFrame.contentWindow.addEventListener( 'afterprint', afterPrintListener( resolve ) );
    });
    
    this.iFrame.focus();
    // await this.iFrame.contentWindow.print();
    await this.pdfViewerApplication.triggerPrinting(); // see Mozilla app.js
    // wait for printing to be finished
    await afterPrintPromise;
    this.iFrame.contentWindow.removeEventListener( 'afterprint', afterPrintListener ); 

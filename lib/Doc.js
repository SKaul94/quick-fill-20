...
export class PdfDoc extends EventTarget  {
    ...
    this.pdfViewerApplication.eventBus.on( "annotationeditorstateschanged", event => {
      ...
      if ( event.details.hasSelectedEditor && event.details.isEditing ){
        ...
        if ( editor.constructor.name === 'StampEditor' ){
          ...
          editor.div.style['z-index'] = 30; // PushButtonWidgetAnnotationElement has z-index 22
          // set the size of the image
          let pushButton = this._pushButton; // section.linkAnnotation.buttonWidgetAnnotation.pushButton
          const allPushButtons = this.iFrame.contentDocument.querySelectorAll('.pushButton');
          const container = pushButton?.container || [...allPushButtons].find( btn => btn.title.match( /Bild|Image/i) );
          // adjust size to container size
          if ( container ) editor.div.style = container.style.cssText;
        }
      }
    });
  ...
  contentSum(){ // concatenate contents of all fields
    return Object.values( this._pdfFieldsDict ).map( 
      f => f._value?.trim() ? `${f.name}=${f._value}` : '' ).join(','); 
  }

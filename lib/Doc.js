...
export class PdfDoc extends EventTarget  {
  ...
  installListeners(){
    ...
    this.iFrame.contentWindow.PDFViewerApplication.eventBus.on( "updateviewarea", event => {
      ...
      if ( this.isReady() ) this.autoLayout();
    } );
  ...
  static updateAllFieldsArea(){
    ...
    const allFields = new Set( PdfDoc.all.map( pdfDoc => Object.values( pdfDoc._pdfFieldsDict )).flat().filter( field => field.typ ) );
  ...
  applyPdfToTable(){
    if ( config.log ) console.log('applyPdfToTable', this.fileName );
  ...
  static async allAreReady(){
    return Promise.all( PdfDoc.all.map( pdfDoc => pdfDoc.isReady() ) );
  }
  ...
  async isReady(){
    ...
    await Promise.race( [
      this.isFullyRenderedPromiseOrTimeout,
      new Promise( resolve => setTimeout( resolve, 100 ) )
    ] ); 
    if ( config.log ) console.log(`============= wait for READY: ${this.fileName} ${this.person.name} ====================`);
    return true;
  }

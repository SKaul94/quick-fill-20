...
export class PdfDoc  {
  ...
  /**
   * @summary final cleanup after PDF document has been closed and HTML DIV element has been removed
   */
  clear(){
    this.pdfViewerApplication.close();
    this.file = null;
  ...
  async renderArrayBuffer(){
    ...
    await new Promise(resolve => setTimeout(resolve, 100)); // ToDo find later event than webviewerloaded
    ...
    if ( ! webviewerloaded && ! this.iFrame.contentWindow.PDFViewerApplication ){
      ...
      await new Promise(resolve => setTimeout(resolve, 1000)); // Timeout needed
    } 

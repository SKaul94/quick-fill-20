...
export class PdfDoc extends EventTarget  {
  ...
  constructor( file, htmlSpace, person ) {
    ...
    console.assert( this.iFrame );  // need to wait for iFrame.load event ? 
    this._docStates = [];
    ...
    const readyStateListener = async event => {
      ...
      if ( this.iFrame.contentDocument.readyState === 'complete' ){
        event.target.removeEventListener('readystatechange', readyStateListener );
        if ( ! this.docState.includes('loaded') ){
          if ( ! this.iFrame.contentWindow.PDFViewerApplication ) await this.waitForState( 'constructed' );
          console.assert( this.docState.includes( 'constructed' ) ); 
          this.startApp();
        }
      }
    };
    ...
    // Therefore try a second event, window.onload 

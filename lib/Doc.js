...
export class PdfDoc extends EventTarget  {
  ...
  constructor( file, htmlSpace, person ) {
    ...
    const readyStateCompleteListener = async event => {
      this.docState = this.iFrame.contentDocument.readyState;
      if ( this.iFrame.contentDocument.readyState === 'complete' ){
        this.iFrame.contentDocument.removeEventListener('readystatechange', readyStateCompleteListener );
        if ( ! this.docState.includes('loaded') ){
          if ( ! this.iFrame.contentWindow.PDFViewerApplication ) await this.waitForState( 'constructed' );
          console.assert( this._docStates.includes( 'constructed' ) ); 
          this.startApp();
        }
      }
    };
    this.iFrame.contentDocument.addEventListener('readystatechange', readyStateCompleteListener );
    ...
    this.isFullyRenderedPromise = this.isFullyRendered || new Promise( resolve => {
      this.addEventListener('ready', async event => { 
        ...
        // Store these images into printImages object: 
        this.pdfDocumentInstance.printImages = [];
    ...
    if ( this.iFrame.contentDocument.readyState === 'complete' ){
      ...
      if ( this.iFrame?.contentWindow?.PDFViewerApplication ){
        ...
        debugger;
        this.waitForState('complete').then( this.startApp );
        debugger;
        setTimeout( this.startApp , 30 );
      }
    }
  }
  ...
  static allStates(){
    console.log( PdfDoc.all.map( pdfDoc => pdfDoc.state() ) );
  }
    ...
    this.pdfViewerApplication.eventBus.on( 'sandboxcreated', async event => {
      ...
      for ( let pageNumber = this.pdfViewerApplication.pagesCount; pageNumber >= 1; pageNumber--){
        try {
          ...
          console.error( `Error Page ${pageNumber}: ${error}` );
        }
      }

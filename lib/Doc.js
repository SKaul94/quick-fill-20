...
export class PdfDoc  {
  ...
  *nextNode(){
  ...
  applyTableToPDF(){
      ...
      if ( fieldObject.htmlElements.find( el => el.hasAttribute('disabled') ) ){
        if ( ! config.ignoreDisabledFields ){
          for ( const htmlElement of fieldObject.htmlElements ){
            console.log(`"disabled" removed from ${htmlElement.tagName} ${htmlElement.name}`);
            htmlElement.removeAttribute("disabled");
            htmlElement.removeAttribute("hidden");
            this.disabledElements.push( htmlElement );
          }
        }
      } 
    ...
    // console.log(`disabledElements = ${this.disabledElements.map(el => el.name)}`);
  } 

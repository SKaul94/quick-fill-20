...
export class RuleEngine{
  ...
  final( field ){
    ...
    if ( result && result.trim().length > 0 && this._ruleChain.length > 0 ){
      ...
      if ( globalThis[ 'document' ] ){  // run in the browser
        if ( config.removeDuplicatesInFieldRulesList ){
          for ( const listItem of document.querySelectorAll('.reason-list li') ){
            const dist = distance( listItem.innerHTML, printFieldRules );
            // if ( dist > 0 && listItem.innerHTML.match(/<b>"(.+)"<\/b>/)[1] === printFieldRules.match(/<b>"(.+)"<\/b>/)[1] ){
            //   console.log( 'listItem.innerHTML = ', listItem.innerHTML );
            //   console.log( 'printFieldRules = ', printFieldRules );
            //   console.log( 'distance = ', dist );
            //   console.log( 'diff = ', listItem.innerHTML.length - printFieldRules.length );
            // } 
            if ( dist < 3 ){  // different persons or different HTML encoding 
              printFieldRules = null; // already in list, do not print a second time
              break;
            }
          }
        }
        if ( printFieldRules ){
          ...
          listItem.innerHTML = printFieldRules;
          document.querySelector('.reason-list').append( listItem );
        }
      } else { // run tests in Node.js
  ...
  * oneStepValueFromAllRules(field){
    ...
    if ( ruleList ) {
      const personalRules = ruleList.filter(rule => rule.value && rule.isPersonRule()).sort((a, b) => a.nr - b.nr);
      const caseRules = ruleList.filter(rule => rule.value && rule.isCaseRule()).sort((a, b) => a.nr - b.nr);
      let specificRules = ruleList.filter(rule => rule.value && !rule.isCaseRule()).sort((a, b) => a.nr - b.nr);
      const pdfSpecificRules = specificRules.filter(rule => rule.pdf?.split(',').find(p => pdfName?.match(p.trim())));
      specificRules = specificRules.filter(rule => !rule.pdf);

      if (pdfSpecificRules.length) {  // 1. PDF-specific rules first
      ...
      } else if (personalRules.length + caseRules.length + specificRules.length) {  // 2. equal-Rules
      ...
      } else {  // 3. iterate all rules
        for (const rule of this._ruleDatabase.sortedRules()) { // already sorted: case rules first
          const value = this.oneStepValueFromRule(field, rule);
          if (value && atomicString !== value) { // a replacement happened
            this.pushToRuleChain(rule);
            yield value;
    ...
    } else {  // 4. iterate all rules if there are no specific rules and no PDF exceptions
      for (const rule of this._ruleDatabase.sortedRules()) { // already sorted: case rules first
        const value = this.oneStepValueFromRule(field, rule);
        if (value && atomicString !== value) { // a replacement happened
          this.pushToRuleChain(rule);

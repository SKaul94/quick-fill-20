...
export class RuleEngine{
   ...
   */
  static firstFinal( ruleDatabase, field, pdfName ){
    const engine = new RuleEngine( ruleDatabase, field, pdfName );
    return engine.final( field, pdfName );
  }
  ...
  final( field, pdfName ){
    ...
    this._origin = source;
    // if ( config.debug ) console.log(`allReplacementsAsArray( '${source}' ) = `, this.allReplacementsAsArray( field, pdfName ) );
    const result = this.finalGen( source, pdfName ).next().value;
      ...
      if ( globalThis[ 'QuickFillStatistics' ] ){
        ...
        if ( this._field.pdfDoc ){
          ...
          if ( ! QuickFillStatistics[ pdfName ].fields.includes( field.fieldName ) ){  // once only
            ...
            // if ( config.debug ) console.log( `${QuickFillStatistics[ pdfName ].countFields}. Feld: ${field.fieldName}, Value: ${result} mit ${result.length} Zeichen wegen `, this._history );
          }
        }
      }
  ...
  * finals( source, pdfName ){
    this._history = [];
    this._level = 0;
    pdfName ||= this._pdfName;
    yield* this.finalGen( source, pdfName );
  }
  finalsArray( source, pdfName ){
    pdfName ||= this._pdfName;
    return Array.from( this.finals( source, pdfName ) );
  }
  allReplacements( source, pdfName ){
    if ( ! this._allReplacements ){
      this._allReplacements = this.allReplacementsAsArray( source, pdfName );
    }  
    return this._allReplacements;
  }
  * finalGen2( source, pdfName ){  // traverse all nested levels until final
    if ( source.constructor.name === 'Field' ) source = source.toSource();
    pdfName ||= this._pdfName;
    this._history.push( source );
    for ( const next of this.allReplacements( source, pdfName ) ){  // traverse one level
  ...
  * finalGen( source, pdfName ){  // traverse all nested levels until final
    if ( source.constructor.name === 'Field' ) source = source.toSource();
    pdfName ||= this._pdfName;
    this._history.push( source );
    // const allReplacements = Array.from( this.replacementsGen( source ) );  // debug only
    // if ( config.debug ) console.log(`replacementsGen( '${source}' ) = `, allReplacements );
    for ( const next of this.replacementsGen( source, pdfName ) ){  // traverse one level
      if ( this.isFinal( next ) ){
        this._history.push( next );
        yield next;
      } else {
  ...
  allReplacementsAsArray( field, pdfName ){
    pdfName ||= this._pdfName;
    return Array.from( this.allGen( field, pdfName ) );
  }

  * allGen( source, pdfName ){  // traverse all nested levels, non-final included
    pdfName ||= this._pdfName;  
  ...
  * replacementsGen( source, pdfName ){  // one level only
    pdfName ||= this._pdfName;
    ...
    for ( const container of allContainers ){
      ...
      for ( const variable of variables ){
        ...
        for ( const value of this.oneStepValueFromAllRules( variable, pdfName ) ){
   ...
   * @param {Field} field - simple variable name used as left hand side of rule substitution
   * @returns {String} value - of given key. The result may include other variables.
   */
  * oneStepValueFromAllRules( field, pdfName ){
    pdfName ||= this._pdfName;
    let atomicString = field;
    ...
    if ( ruleList ) {
      const headerRules = Object.values( Person.currentActive?.headerRules || {} ).filter( rule => rule.value ); 
      ...
      if ( headerRules.length ) {  // 1. Header rules with highest priority
        for ( const rule of headerRules ){
          const value = this.oneStepValueFromRule(field, rule, pdfName);
  ...
  /**
   * @summary prevents the application of the given rule
   * @param {Field} field - field used as left hand side for rule substitution
   * @param {Rule} rule - rule to be inspected
   * @returns {Boolean} value - true iff the application of the given rule should be prevented
   */
   ...
   * @param {Field} field - left hand side of rule substitution
   * @param {Rule} rule - rule in Rule.DB
   * @returns {String} value - computed by applying subRule to field
   */
  oneStepValueFromRule(field, rule, pdfName) {
    pdfName ||= this._pdfName;
    ...
    if ( rule.pdf ){
      ...
      const restrictionApplies = () => {
        for ( const pdfName of allPdfRestrictions ){
          if ( field.fileName?.includes( pdfName ) ) return true;
          if ( this._pdfName?.includes( pdfName ) ) return true;
        }
        return false;
      }
  ...
  replacementsArray( source, pdfName ){
    pdfName ||= this._pdfName;
    this._history = [];
    this._level = 0;
    this._origin = source;
    return Array.from( this.allGen( source, pdfName ) );
  }
   ...
   */
  maxReplaced( source, pdfName ){
    pdfName ||= this._pdfName;
    source = source.constructor.name === 'Field' ? '${' + source.fieldName + '}$' : source;
    const allReplaced = this.replacementsArray( source, pdfName );

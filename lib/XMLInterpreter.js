...
let filenameArchiveMapping = null;
export function pdfArchiveOf( fileName ){
  if ( ! filenameArchiveMapping ){
    filenameArchiveMapping = {};
    for ( const [ archiveName, fileMapping ] of Object.entries( config.pdfPerPerson ) ){
      for ( const mappedFile of Object.values( fileMapping ) ){
        filenameArchiveMapping[ mappedFile ] = `${archiveName}.zip`;
      }
    }
  }
  return filenameArchiveMapping[ fileName ];
}

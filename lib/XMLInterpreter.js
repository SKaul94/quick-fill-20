...
export class XMLInterpreter {
    ...
    defaultValue( key, title ){
        return '';
        // return Rule.DB.valueFromAllRulesOf( title );
    }
    ...
    async makeMultiplePdfs( person, selectedLanguage, personDiv ){
        ...
        if ( config.headerRules ){
            const personHeader = document.createElement('div');
            personHeader.classList.add('person_header', 'center');
            let personHeaderHTML = '';
            for ( const [ key, title ] of Object.entries( config.headerRules ) ){
                personHeaderHTML += `<fieldset class="inline"><legend>${title}</legend><input placeholder="${title}" class="${key} large" type="text"></fieldset>`;
            }
            ...
            for ( const [ key, title ] of Object.entries( config.headerRules ) ){
                const input = personHeader.querySelector(`input.${key}`);
                person.headerRules[ key ] = Rule.createNew({
                    rule_type: 'equal', 
                    rule: `Header-${title}`, 
                    value: this.defaultValue( key, title ),
                    owner: 'case',
                    person
                });
                input.addEventListener('input', event => {
                    person.headerRules[ key ].value = event.target.value;

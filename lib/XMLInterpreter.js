...
import { createStore, get } from './idb-keyval.js';
...
export class XMLInterpreter {
    ...
    async interpret( event ){
        ...
        if ( this.xmlDOM ){
            ...
            if ( this.allPersons ) for ( const person of this.allPersons ){
                ...
                this.addRuleFor( nachname, 'familienname', personName ); 
                ...
                // geburtsdatum + 18 Jahre >= heute ? 'asyl' : 'minder';
                const kindOfPDF = geburtsdatum && new Date(this.norm(geburtsdatum)).getFullYear() + 18 >= new Date().getFullYear() ? 'asyl' : 'minder';

                // create PDF from binary
                const pdfDoc = new PdfDoc( {name: kindOfPDF}, selected_content );

                // fill PDF with data from XML and show in div.selected_content
                let binary = globalThis.pdfBinary && globalThis.pdfBinary[ kindOfPDF ];
                if ( ! binary ){
                    const language = 'englisch';
                    const key = kindOfPDF + '_' + language;
                    binary = await get(key); // fetch from IndexedDB
                } 
                if ( binary ){
                    selected_content.querySelector('.filename').innerText = binary.filename;
                    await pdfDoc.renderArrayBuffer( binary );
                    pdfDoc.applyRulesToTable( personName ); // ToDo
                    pdfDoc.applyTableToPDF();   
                } else {
                    alert( `Bitte vorher passendes PDF für ${kindOfPDF} laden` );
                } 
                await new Promise( resolve => setTimeout(resolve, 1000) );
                selected_content.classList.add('hide');  // ToDo class hide
             }
        }
    ...
    addRuleFor( xmlField, xmlLongName, personName ){

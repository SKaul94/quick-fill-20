...
export class XMLInterpreter {
    ...
    async interpret( event ){
        ...
        // this.proposedEncoding = JsCharDet.jschardet.detect( arrayBuffer.slice(0,Math.min(1024,arrayBuffer.byteLength)) );
        ...
        // const encoding = proposedEncoding.confidence > 0.5 ? proposedEncoding.encoding : 'utf8';
        const encoding = proposedEncoding || config.xmlEncoding;
    ...
    async makeMultiplePdfs( person ){
        const pdfPerPerson = config.pdfPerPerson ? config.pdfPerPerson : { "PDF": null };

...
function pdfArchiveOf( fileNames ){
  for ( const [ archiveName, fileMapping ] of Object.entries( config.pdfPerPerson ) ){
    for ( const mappedFiles of Object.values( fileMapping ) ){
        if ( Array.isArray( fileNames ) && Array.isArray( mappedFiles ) ){
            if ( fileNames.sort().toString() == mappedFiles.sort().toString() ) return `${archiveName}.zip`;
        } 
        if ( ! Array.isArray( fileNames ) && ! Array.isArray( mappedFiles ) ) {
            if ( nameMatch( fileNames, mappedFiles ) ) return `${archiveName}.zip`;
        }   
    }
  }
}

function nameMatch( fileNameWithSuffix, mappedFile ){
    const fileName = fileNameWithSuffix.at(-4) === '.' ? fileNameWithSuffix.slice(0,-4) : fileNameWithSuffix; 
    if ( mappedFile?.match( fileName ) ) return true;
    return false;
}
...
export class XMLInterpreter {
    ...
    async interpret(){
        ...
        if ( this.xmlDOM ){
            ...
            if ( this.allPersons ) loop: for ( const person of Person.all() ){
                ...
                await person.activate();
                let { div: selectedPersonDiv, pdfs } = await this.makeMultiplePdfs( person );
                ...
                const languageChangeListener = async event => {
                    ...
                    const selectedPersonParent = selectedPersonDiv.parentElement;
                    const personEditor = selectedPersonDiv.querySelector('.person.editor').parentElement;
                    ...
                    const result = await this.makeMultiplePdfs( person );
                    // append old person editor with old values stored inside
                    result.div.appendChild( personEditor );
                    
                    selectedPersonParent.querySelector('.active')?.classList.remove('active');
                    // selectedPersonParent.appendChild( result.div );  // This does not work: PDFs must not be re-positioned
                    
                    // set value of language selector
                    pdfs = result.pdfs;
                    const pdfDoc = pdfs[0];
                ...
                pdfs[0].htmlSpace.querySelector('select.language_selector')?.addEventListener('change', languageChangeListener);
    ...
    async makePdf( person, htmlSpace, pdfFiles ){
        const selected_pdf = PdfDoc.createHTMLSpace();
        const pdfFileKey = Array.isArray( pdfFiles ) ? this.pdfFileKey( selected_pdf, person, pdfFiles ) : pdfFiles;
        ...
        personal_language_selector.id = makeClassName('language_selector_' + person.nachname + '_' + pdfFileKey);
     ...
     */
    pdfFileKey( selected_content, person, kindsOfPDFs ){
    ...
    async appendPdf( pdfFileKey, selected_content, person ){
        ...
        if ( ! pdfBinary ){
            ...
            try {
                await loadAndDecryptArchive( archive );
                pdfBinary = await Idb.get( pdfFileKey );
            } catch (error) {
                console.error( error );
                alert( `Bitte zuerst passendes PDF "${pdfFileKey}" laden!` );
                return;
            } 
        }

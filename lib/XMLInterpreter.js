...
import {filename_language_mapper, makeClassName, shareSVG} from './global_functions.js';
...
export class XMLInterpreter {
    ...
    async interpret(){
        ...
        if ( this.xmlDOM?.body?.innerHTML?.match(/parsererror/i) ){
            console.assert( false, this.xmlDOM.body.innerHTML );
            alert(`Fehler in XML.`);
            this.xmlDOM = null;
        }
    ...
    async makeMultiplePdfs( person, selectedLanguage, personDiv ){
        ...
        if ( config.summaryButton ){
            ...
            menuNode.addEventListener( 'click', async event => {
                ...
                if ( event.target.classList.contains('active') ){
                    ...
                    mergedPdf.innerHTML = `<h1>Druckvorschau aller PDFs von ${person.name} hintereinander</h1><ol></ol>`;
        ...
        if ( config.qrButton ){
            const menuNode = document.createElement('li');
            menuNode.innerText = config.qrButtonLabel;
            menuNode.href = '#';
            menu.appendChild( menuNode );
    
            const qrCodesDiv = document.createElement('div'); 
            qrCodesDiv.classList.add('selected_pdf');      
            selected_person_space.appendChild( qrCodesDiv );
            ...
            menuNode.addEventListener( 'click', async event => {
                ...
                if ( event.target.classList.contains('active') ){
                    event.target.classList.remove('active');
                    selected_person_space.querySelectorAll('.selected_pdf').forEach( selected_pdf => selected_pdf.classList.add('hidden') );
                    qrCodesDiv.querySelectorAll('div.qr_code').forEach( img => img.classList.add('hide') );
                } else {
                    menuNode.parentNode.querySelectorAll('li').forEach( li => li.classList.remove('active') );
                    menuNode.classList.add('active');
                    selected_person_space.querySelectorAll('.selected_pdf').forEach( selected_pdf => selected_pdf.classList.add('hidden') );
                    qrCodesDiv.classList.remove('hidden');
                    qrCodesDiv.innerHTML = `<h1>QR-Codes aller PDFs von ${person.name} hintereinander</h1>`;

                    for ( const pdfDoc of person.allPdfs ){
                        const div = document.createElement('div');
                        div.classList.add('qr_code');
                        div.innerHTML = `<h2>QR-Code für "${pdfDoc.title}" von "${person.name}"</h2><svg class="qr_svg" width="100%"></svg>`;
                        const svgElement = div.querySelector('svg');
                        pdfDoc.qrCodeSvg( svgElement, pdfDoc.contentSum() );
        ...
        const menuNode = document.createElement('li');
        menuNode.innerHTML = shareSVG;
        menuNode.href = '#';
        menuNode.classList.add('clickable')
        menu.appendChild( menuNode );

        menuNode.addEventListener( 'click', async event => {
            const files = [];
            for ( const personPdfDoc of person.allPdfs ){
                const blob = new Blob([ personPdfDoc.pdfDocumentInstance ]);
                const file = new File([ blob ], `${personPdfDoc.key}.pdf`, { type: "application/pdf" }); 
                files.push( file );
            }
            const shareData = {
                files,
    ...
    async makePdf( person, htmlSpace, pdfFiles, selectedLanguage ){
        ...
        if ( config.personEditor && ! person.profileEditor ){

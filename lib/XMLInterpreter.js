...
export class XMLInterpreter {
    ...
    switchPersonalRules( person ){
        ...
        Rule.DB.forEach( rule => {
            rule.active = rule.person ? rule.person === person : true;
        });
    }
    ...
    generatePersonalRules( person ){
        ...
        for ( const key of Object.keys( config.xmlPersonalRules ) ){
            ...
            this.addRuleFor( person.data[key], personalRule, person ); 
        }
     
        // add rules from Map for this person
        for ( const [xmlField, xmlLongName] of this.xmlFieldLongNameMap.entries() ){
            if ( xmlField.person && xmlField.person === person ){
                this.addRuleFor( xmlField, xmlLongName, person );  // include all XML data of this person
                for ( const key of Object.keys( config.xmlKeyValuePairs ) ){
                    if ( xmlField.textContent === key ){
                        const textContent = xmlField.nextElementSibling.textContent;
                        this.addRuleFor( { textContent }, config.xmlKeyValuePairs[key], person ); 
                    }
    ...
    async makePdf( person, language ){
        ...
        for ( const rule of config.initialRulesList.template ){
            new Rule({rule_type: 'equal', rule: rule.rule, value: '', owner: 'person', person });
        }
    ...
    async appendPdf( pdfFileKey, selected_content, person ){
        ...
        if ( pdfBinary ){
            ...
            pdfDoc.applyRulesToTable( person ); // ToDo personalized rules
        } else {
    ...
    addRuleFor( xmlField, xmlLongName, person ){
        ...
        if ( oldRule && oldRule.active && oldRule.owner !== 'template' && ( person ? oldRule.person === person : ! oldRule.person ) ){
          oldRule.value = xmlData;
          return oldRule;
        }
        if ( xmlData && xmlData.length > 0 && ! config.ignoreXMLFields.includes( rule ) ){
          return new Rule({rule, value: xmlData, rule_type: 'superstring', owner: 'person', person });
        }
    }
}

...
function pdfArchiveOf( fileName ){
  for ( const [ archiveName, fileMapping ] of Object.entries( config.pdfPerPerson ) ){
    for ( const mappedFile of Object.values( fileMapping ) ){
        if ( nameMatch( fileName, mappedFile ) ) return `${archiveName}.zip`;
    }
  }
}

function nameMatch( fileNameWithSuffix, mappedFile ){
    const fileName = fileNameWithSuffix.slice(0,-4);
...
export class XMLInterpreter {
    constructor( button, htmlSpace ) {
        ...
        div.classList.add( 'invoke' + this.count );
    ...
    async makePdf( person, htmlSpace, pdfFile ){
        ...
        personal_language_selector.id = 'language_selector_' + person.nachname + '_' + pdfFileKey;
    ...
    async appendPdf( pdfFileKey, selected_content, person ){
        ...
        if ( ! pdfBinary ){
            ...
            if ( confirm( `Zuerst passendes PDF ${pdfFileKey} laden?` ) ){
                const archive = pdfArchiveOf( pdfFileKey );
                console.log(`Passendes Archiv gefunden: ${archive}`);
                if ( ! archive ){ alert('Kein passendes Archiv gefunden.'); return; }
                try {

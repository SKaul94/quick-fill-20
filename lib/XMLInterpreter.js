    ...
    async interpret( event ){
        ...
        if ( this.xmlDOM ){
            ...
            if ( this.allPersons ) for ( const person of this.allPersons ){
                ...
                const language = document.querySelector('select#language_selector')?.value || 'englisch';
                let pdfDoc = await this.makePdf( nr, personName, language, birthDate );

                pdfDoc.htmlSpace.querySelector('select.language_selector')?.addEventListener('change', async event => {
                    pdfDoc.remove();
                    const language = event.target.value;
                    pdfDoc = await this.makePdf( nr, personName, language, birthDate ); 
                    const pdfFileKey = this.pdfFileKey( pdfDoc.htmlSpace, birthDate );

...
export class XMLInterpreter {
    ...
    async makeMultiplePdfs( person ){
        if ( ! config.pdfPerPerson ) return this.makePdf( person, this.htmlSpace );
        ...
        const allPdfsOfPerson = [];

        const classForPdf = ( pdfFile, pdfDoc ) => pdfFile ? 'Pdf'+pdfFile : pdfDoc.pdfFileKey; 
 
        for ( const [ pdfName, pdfFile ] of Object.entries( config.pdfPerPerson ) ){
            ...
            menuNode.addEventListener('click', async event => {
                ...
                if ( event.target.classList.contains('active') ){
                    ...
                    // selected_person_space.querySelector('.'+ classForPdf( pdfFile, pdfDoc)).classList.remove('hidden');
                }
            } );
        }
        selected_person_space.classList.add('hidden');
        return { div: selected_person_space, pdfs: allPdfsOfPerson };
    }
    ...
    async makePdf( person, htmlSpace, pdfFile ){
        ...
        // selected_pdf.appendChild( personDataEditorSpace );
    ...
    async appendPdf( pdfFileKey, selected_content, person ){
        ...
        if ( pdfBinary ){
            ...
            await new Promise( resolve => setTimeout( resolve, 100 ) ); // wait for textlayerrenderedPromise to finish
        } else {
